          ...
          <div class="px-4 py-3 border-bottom">
              <div class="row">
                  ...
                  <div class="col text-end d-flex justify-content-end">
                      <a href="{{ route('penyidikan.create') }}" class="btn btn-primary d-flex align-items-center">
                        Tambah {{ $title }}
                      </a>
                  </div>
                  {{-- @endcan --}}
...
@push ('after-scripts')
    <script>
        var table = $('#table_yajra').DataTable({
            ...
            pageLength: 5,
            ajax: {
                "url": "{{ route('penyidikan.datatable') }}",
                "type": "POST",
        ...
        function destroy(uuid) {
            var url = "{{ route('penyidikan.destroy', ':uuid') }}".replace(':uuid', uuid);
